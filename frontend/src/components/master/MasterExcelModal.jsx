...
export default function MasterExcelModal({ open, onClose, title, subtitle, onDownloadSample, onUpload }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} subtitle={subtitle} size="lg">
      <div className="space-y-4">
        <div className="rounded-md border border-blue-100 bg-slate-50 p-4 text-sm text-slate-600">
          Download the sample file, keep the same column names, fill your rows, and upload the completed Excel file.
        </div>
        <div className="grid gap-3 rounded-md bg-slate-50 p-4 md:grid-cols-2">
          <Button variant="secondary" onClick={onDownloadSample}><Download size={16} /> Download Sample Excel</Button>
          <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-md bg-[#0B1220] px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-blue-600/20 hover:bg-[#111C33]">
